...
public class BinaryTree<E extends Comparable> {
	...
	public void listOfDepths(LinkedList<LinkedList<BinaryTreeNode<Integer>>> listOfNodes) {
		this.checkDepths(0,this.root);
		this.preOrderAddToList(listOfNodes,this.root);
	}
	private void preOrderAddToList(LinkedList<LinkedList<BinaryTreeNode<Integer>>> listOfNodes, BinaryTreeNode node) {
		// TODO Auto-generated method stub
		if(node==null) {
			return;
		}
		listOfNodes.get(node.depth).add(node);
		this.preOrderAddToList(listOfNodes, node.left);
		this.preOrderAddToList(listOfNodes, node.right);
